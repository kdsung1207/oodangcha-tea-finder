export const TEAS = ["루이보스차", "홍차", "보이차", "우롱차", "말차", "호지차", "히비스커스티"] as const;
...
export const TEA_INFO: Record<Tea, { copy: string; note: string; color: string; soft: string }> = {
  루이보스차: { copy: "오늘의 속도를 다독이는 편안한 한 잔", note: "부드럽고 맑은 여운", color: "var(--tea-rooibos)", soft: "var(--tea-rooibos-soft)" },
  홍차: { copy: "분주한 하루에 깊이를 더하는 한 잔", note: "풍부하고 클래식한 향", color: "var(--tea-black)", soft: "var(--tea-black-soft)" },
  보이차: { copy: "묵직한 중심이 필요한 오늘을 위한 한 잔", note: "깊고 차분한 풍미", color: "var(--tea-puer)", soft: "var(--tea-puer-soft)" },
  우롱차: { copy: "기분 좋은 집중을 오래 이어주는 한 잔", note: "향긋하고 산뜻한 밸런스", color: "var(--tea-oolong)", soft: "var(--tea-oolong-soft)" },
  말차: { copy: "선명한 에너지로 오늘을 깨우는 한 잔", note: "쌉쌀하고 생생한 녹음", color: "var(--tea-matcha)", soft: "var(--tea-matcha-soft)" },
  호지차: { copy: "고소한 온기로 하루를 감싸는 한 잔", note: "구수하고 편안한 향", color: "var(--tea-hojicha)", soft: "var(--tea-hojicha-soft)" },
  히비스커스티: { copy: "산뜻한 전환이 필요한 순간의 한 잔", note: "상큼하고 생기 있는 맛", color: "var(--tea-hibiscus)", soft: "var(--tea-hibiscus-soft)" },
};
...
export const QUESTIONS: QuizQuestion[] = [
  ...
  { title: "음료를 고를 때 가장 중시하는 부분은?", options: [
  ...
  { title: "음료를 가장 많이 마시는 시간은?", options: [
  ...
  { title: "평소 차를 잘 마시지 않는 이유는?", options: [
  ...
  { title: "새로운 음료를 고를 때 끌리는 맛은?", options: [
...
export function calculateTeaResult(answers: number[]): Tea {
  ...
  answers.forEach((answer, questionIndex) => {
    ...
    TEAS.forEach((tea) => { scores[tea] += weights[tea] ?? 0; });
  });
  return TEAS.reduce((best, tea) => scores[tea] > scores[best] ? tea : best, TEAS[0]);
}

export function buildReason(answers: number[], tea: Tea) {
  const labels = answers.map((answer, index) => QUESTIONS[index]?.options[answer]?.label).filter(Boolean);
