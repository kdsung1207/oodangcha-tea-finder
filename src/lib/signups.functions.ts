...
function token(secret: string) {
  return createHmac("sha256", secret).update("odangcha-admin-v1").digest("hex");
}
...
export const getSlotCounts = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return TIMESLOTS.map((time) => ({
    time,
    count: data.filter((row) => row.timeslot === time).length,
...
export const submitSignup = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: inserted, error } = await supabaseAdmin.rpc("reserve_tea_timeslot", {
      ...
      p_tea_result: data.teaResult,
      p_timeslot: data.timeslot,
      p_capacity: SLOT_CAPACITY,
    });
    if (error?.message.includes("timeslot_full"))
      throw new Error("선택한 시간대가 방금 마감되었어요.");
...
export const getAdminSignups = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const { data, error } = await supabaseAdmin
    .from("signups")
    .select("id,name,contact,tea_result,timeslot,created_at")
    .order("timeslot")
    .order("created_at");
