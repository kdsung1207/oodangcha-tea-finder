...
export function BrewAnimation({ tea, onDone }: { tea: Tea; onDone: () => void }) {
  ...
  const duration = reduced ? 0.4 : 3.2;
  return (
    <motion.div
    ...
    >
      ...
      <div className="relative h-80 w-full max-w-sm">
        <motion.div
          ...
          transition={{ duration: reduced ? 0.2 : 0.8, delay: 0.25, ease: "easeIn" }}
        >
          차고
        </motion.div>
        <div className="absolute bottom-4 left-1/2 h-52 w-64 -translate-x-1/2 overflow-hidden rounded-b-[4.5rem] border-2 border-brand-deep/20 bg-card/40 shadow-lift">
          ...
          <motion.div
            className="absolute bottom-0 left-1/2 h-20 w-20 -translate-x-1/2 rounded-full blur-xl"
            style={{ background: TEA_INFO[tea].color }}
            initial={{ scale: 0.05, opacity: 0 }}
            animate={{ scale: 5, opacity: 0.82 }}
            transition={{
          ...
          <motion.div
            ...
            style={{ background: TEA_INFO[tea].soft }}
            initial={{ opacity: 0 }}
            animate={{ opacity: 0.72 }}
            transition={{ duration: reduced ? 0.2 : 1.5, delay: reduced ? 0.1 : 1.5 }}
          />
        </div>
      </div>
      <motion.p
